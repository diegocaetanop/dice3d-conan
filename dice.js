...
(function(dice) {
    ...
    this.known_types = ['d6red','d6orange', 'd6yellow'];
    this.dice_face_range = { 'd6': [1, 6] };
    this.dice_mass = { 'd6red': 300, 'd6orange': 300, 'd6yellow': 300 };
    this.dice_inertia = { 'd6red': 13, 'd6orange': 13, 'd6yellow': 13 };

    this.scale = 50;

    this.create_d6red = function(dice_type) {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.9);
        if (!this.dice_material || (typeof dice_type != 'undefined' && dice_type.search('/red/'))) {
            this.dice_material = new THREE.MeshFaceMaterial(
                this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, known_colors['red']));
        }
        return new THREE.Mesh(this.d6_geometry, this.dice_material);
    }

    this.create_d6orange = function(dice_type) {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.9);
        if (!this.dice_material || (typeof dice_type != 'undefined' && dice_type.search('/orange/'))) {
            this.dice_material = new THREE.MeshFaceMaterial(
                this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, known_colors['orange']));
        }
        return new THREE.Mesh(this.d6_geometry, this.dice_material);
    }

    this.create_d6yellow = function(dice_type) {
        if (!this.d6_geometry) this.d6_geometry = this.create_d6_geometry(this.scale * 0.9);
        if (!this.dice_material || (typeof dice_type != 'undefined' && dice_type.search('/yellow/'))) {
            this.dice_material = new THREE.MeshFaceMaterial(
                this.create_dice_materials(this.standart_d20_dice_face_labels, this.scale / 2, 1.0, known_colors['yellow']));
        }
        return new THREE.Mesh(this.d6_geometry, this.dice_material);
    }
    ...
    this.parse_notation = function(notation) {
        ...
        var dr0 = /\s*(\d*)([a-z]+)(\d+[a-z]+)(\s*\+\s*(\d+)){0,1}\s*(\+|$)/gi;
        var dr1 = /(\b)*(\d+)(\b)*/gi;
    ...
    this.dice_box.prototype.create_dice = function(type, pos, velocity, angle, axis) {
        var dice = that['create_' + type](type);
        dice.castShadow = true;
        dice.dice_type = type;
        dice.body = new CANNON.RigidBody(that.dice_mass[type],dice.geometry.cannon_shape, dice.dice_material);
    ...
    function get_dice_value(dice) {
        ...
        /* conan specifics - red dice */
        /*
        if ( matindex == 1 ) {
            matindex = 0;
        } else if ( matindex == 2 || matindex == 5 ) {
            matindex = 2;
        } else if ( matindex == 3 || matindex == 4 ) {
            matindex = 1;
        } else {
            matindex = 3;
        }
    ...
    this.dice_box.prototype.draw_selector = function() {
        ...
        for (var i = 0, pos = -3; i < that.known_types.length; ++i, ++pos) {
            var dice = $t.dice['create_' + that.known_types[i]](that.known_types[i]);
